...
--%>
		<c:forEach var="temp" items="${LIST}">
			...
			<tr>
				<td>글쓴이</td>
			<%--<td colspan="3"><%= temp.get("WRITER") %></td>--%>
			<td colspan="3">${temp.WRITER}</td>
			</tr>
			<tr>
				<td>본문</td>
			<%--<td colspan="3"><%= temp.get("BODY") %></td>--%>
...
--%>
		<form method="POST" action="../Board/BoardReWrite.bbs" id="refrm">
			<%--<input type="hidden" name="oriNO" value="<%= map.get("NO") %>"> --%>
			<input type="hidden" name="oriNO" value="${DATA.NO}">
			<table width="50%" border="1" align="center">
				<tr>
					<td>글쓴이</td>
			<%--<td><input type="text" name="writer" value="<%= session.getAttribute("ID") %>" disabled></td> --%>	
